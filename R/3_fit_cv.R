...
fit_cv8_limited <- hetero(f8, data = cv_data, panel.id = ~ gwcode + year, method = "BFGS")
fit_cv9_limited <- hetero(f9, data = cv_data, panel.id = ~ gwcode + year, method = "BFGS")
fit_cv10_limited <- hetero(f10, data = cv_data, panel.id = ~ gwcode + year, method = "BFGS")
...
comp <- compare_models(fit_cv1_limited,
											 ...
											 fit_cv8_limited,
											 fit_cv9_limited,
											 fit_cv10_limited, original_data = main_df, test = T)
comp

summary(fit_cv10)
summary(fit_cv10_limited)
